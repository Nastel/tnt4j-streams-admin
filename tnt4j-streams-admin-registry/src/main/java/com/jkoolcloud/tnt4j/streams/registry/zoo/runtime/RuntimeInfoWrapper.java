...
public class RuntimeInfoWrapper {
	...
	private static final Logger logger = LoggerFactory.getLogger(RuntimeInfoWrapper.class);
	...
	private static Map<String, Object> getStreamsToClassMapWrapper() {
		...
		try {
			...
			logger.error(String.format("Failed to extract classes from stream config err msg: %s", e.getMessage()));
		}
