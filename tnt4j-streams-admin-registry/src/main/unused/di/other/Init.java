...
public class Init {
	...
	private static TokenAuth readToken = new TokenAuth();

	private static TokenAuth actionToken = new TokenAuth();
	...
	private void startJetty(Properties properties) {
		...
		servletHandler.setInitParameter("resteasy.providers",
				"com.jkoolcloud.tnt4j.streams.registry.zoo.authentication.ReadRequestFilter,com.jkoolcloud.tnt4j.streams.registry.zoo.authentication.ActionRequestFilter");
	...
	public void createZkTree(Properties properties) {
		...
		for (String node : nodeList) {
			...
			if (nodePathAndEndpoint.length == 2) {
				...
				if (node.equals("readToken") && CuratorUtils.doesNodeExist(nodePathAndEndpoint[0])) {
					CuratorUtils.setData(nodePathAndEndpoint[0], readToken.getIdentifier());
				}

				if (node.equals("actionToken") && CuratorUtils.doesNodeExist(nodePathAndEndpoint[0])) {
					CuratorUtils.setData(nodePathAndEndpoint[0], actionToken.getIdentifier());
				}
			}
		}
	}
