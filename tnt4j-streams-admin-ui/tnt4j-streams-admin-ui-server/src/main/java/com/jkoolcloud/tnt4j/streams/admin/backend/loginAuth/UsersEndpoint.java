...
public class UsersEndpoint {
	...
	public Response doGetRepositoryInfo(@HeaderParam("Authorization") String header, @HeaderParam("test") String test) {
		...
		try {
			...
			String loginResponse = usersUtils.loginTheUserByCredentials(Authorizations.get("username").toString(),
					Authorizations.get("password").toString());
	 ...
	 * 
	 * @param header
	 *            the auth token saved inside the request
	 * @return
	 */
	@GET
	@Path("/logout")
	...
	public Response logoutApproved(@HeaderParam("Authorization") String header) {
		...
		try {
			...
			if (!userStillExist) {
				...
				return Response.status(500).entity("{ \"logout\" : \"There was a problem on logout of the user\" }")
						.build();
			}
			if (header.isEmpty()) {
			...
			return Response.status(500)
					.entity("{ \"logout\" : \"No response from server or server error encountered\" }").build();
		}
		...
		if (usersUtils.checkIfUserExistAndBypassLogin(header, cache)) {
			try {
				...
				return Response.status(200)
						.entity("{ \"Register\" : \"New user " + username + " was added successfully!\" }").build();
			} catch (AuthenticationException e) {
				LOG.error("A user with that username already exists ");
				return Response.status(200).entity(
						"{ \"Register\" : \"The user was not created successfully, the username is already taken!\"  }")
						.build();
			} catch (Exception e) {
				LOG.error("Problem on adding a new user to the user ZooKeeper repository");
				return Response.status(200)
						.entity("{ \"Register\" : \"The user was not created successfully, problem occurred!\"  }")
						.build();
			}
		} else {
			LOG.info("Return a 401 inside tree data call");
			return Response.status(401)
					.entity("{\"Register\" : \"Tried to access protected resources. No token was found\" }").build();
		}
	}
			...
			try {
				...
				try {
					...
					if (!UsersUtils.getUserRemoval()) {
						return Response.status(200).entity("{ \"Remove\" : \"A problem occurred, the user " + username
								+ " was not removed successfully. Please try again later.\"  }").build();
					}
				} catch (Exception e) {
					LOG.info("Not registered, insufficient rights");
					return Response.status(200).entity("{ \"Remove\" : \"The user " + username
							+ " was not removed successfully, insufficient rights!\"  }").build();
				}
				return Response.status(200)
						.entity("{ \"Remove\" : \"The user " + username + " was removed successfully!\" }").build();

			} catch (Exception e) {
				LOG.error("Problem on adding a new user to the user ZooKeeper repository");
				return Response.status(500)
						.entity("{\"Remove\" : \"No response from server or server error encountered\" }").build();
			}
		} else {
			LOG.info("Return a 401 inside tree data call");
			return Response.status(401)
					.entity("{\"Remove\" : \"Tried to access protected resources. No token was found\" }").build();
		}
	}
		...
		if (usersUtils.checkIfUserExistAndBypassLogin(header, cache)) {
			try {
				...
				try {
					...
					return Response.status(200)
							.entity("{ \"Edit\" : \"The user " + username + " was not updated successfully.\"  }")
							.build();
				} catch (AuthenticationException e) {
					LOG.info("Not registered, insufficient rights");
					return Response.status(200).entity("{ \"Edit\" : \"The user " + username
							+ " was not updated successfully, insufficient rights!\"  }").build();
				}
				return Response.status(200).entity("{ \"Edit\" : \"The user " + username + " updated successfully!\" }")
						.build();

			} catch (Exception e) {
				LOG.error("Problem on adding a new user to the user ZooKeeper repository");
				return Response.status(401)
						.entity("{\"Edit\" : \"No response from server or server error encountered\" }").build();
			}
		} else {
			LOG.info("Return a 401 inside tree data call");
			return Response.status(401)
					.entity("{\"Edit\" : \"Tried to access protected resources. No token was found\" }").build();
		}
	}
	...
	public Response getUpdatedUsersList(@HeaderParam("Authorization") String header, String userInformationForEdit) {
		...
		if (usersUtils.checkIfUserExistAndBypassLogin(header, cache)) {
			try {
				...
				LOG.error("Problem on adding a new user to the user ZooKeeper repository");
				return Response.status(500)
						.entity("{\"userList\" : \"No response from server or server error encountered.\" }").build();
			}
		} else {
			LOG.info("Return a 401 inside tree data call");
			return Response.status(401)
					.entity("{\"userList\" : \"Tried to access protected resources. No token was found\" }").build();
		}
	}
